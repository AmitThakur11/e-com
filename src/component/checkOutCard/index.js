...
export default function CheckOutCard({cart}) {
    ...
    const total = cart && cart.reduce((acc , item)=>{
        const {productId : {price , discount},qty} = item
            ...
            <div className ="checkoutProduct">
                    {
                        cart.map((product)=>{
                            return <div className = "checkout" key = {product.productId._id}>
                                <div><strong>{product.productId.name} *</strong><span style={{color:"red"}}>{product.qty} </span> : {(product.productId.price - product.productId.discount)*product.qty} Rs</div>
                            </div>
